...
- `KOD_UPDATE_FEED_SIGNING_KEY_SEED_BASE64` / `KOD_UPDATE_FEED_SIGNING_KEY_ID`: an Ed25519 seed generated
  offline via `swift run --package-path Packages/KodCore UpdateFeedTool generate-key` (mirroring
  `Scripts/managed-install-signing/README.md`'s identical process for the managed-install catalog) — never
  generated by, or stored in, this repository.
