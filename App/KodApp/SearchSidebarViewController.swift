...
@MainActor
final class SearchSidebarViewController: NSViewController {
    ...
    private let searcher: WorkspaceTextSearcher?
    private let engineUnavailableReason: String?
    ...
    private let matchCaseButton = NSButton(
        checkboxWithTitle: Localized.string("Case", comment: "Checkbox toggling case-sensitive workspace search"),
        target: nil,
        action: nil
    )
    private let wholeWordButton = NSButton(
        checkboxWithTitle: Localized.string("Word", comment: "Checkbox toggling whole-word matching in workspace search"),
        target: nil,
        action: nil
    )
    private let regexButton = NSButton(
        checkboxWithTitle: Localized.string("Regex", comment: "Checkbox toggling regular-expression matching in workspace search"),
        target: nil,
        action: nil
    )
    private let includeHiddenButton = NSButton(
        checkboxWithTitle: Localized.string("Hidden", comment: "Checkbox toggling inclusion of hidden files in workspace search"),
        target: nil,
        action: nil
    )
    private let includeIgnoredButton = NSButton(
        checkboxWithTitle: Localized.string("Ignored", comment: "Checkbox toggling inclusion of ignored files in workspace search"),
        target: nil,
        action: nil
    )
    ...
    init(
        ...
        do {
            self.searcher = try WorkspaceTextSearcher()
            self.engineUnavailableReason = nil
        } catch {
            self.searcher = nil
            let reason = Localized.string(
        ...
        if let unavailableReason {
            Task {
                await diagnosticsLog.record(
                    subsystem: .search,
                    level: .warning,
                    message: Localized.string("Workspace search engine failed to initialize", comment: "Diagnostics log message recorded when the workspace text search engine fails to start"),
                    context: [
    ...
    override func loadView() {
        ...
        searchField.placeholderString = Localized.string("Search Workspace", comment: "Placeholder text in the workspace search field")
        ...
        includeGlobField.placeholderString = Localized.string("Include (e.g. *.swift)", comment: "Placeholder text for the include-glob filter field in workspace search")
        ...
        excludeGlobField.placeholderString = Localized.string("Exclude (e.g. Generated/**)", comment: "Placeholder text for the exclude-glob filter field in workspace search")
        ...
        column.title = Localized.string("Results", comment: "Column title for the workspace search results outline view")
    ...
    private func runSearch() {
        ...
        statusLabel.stringValue = Localized.string("Searching…", comment: "Status label shown while a workspace search is in progress")
        statusLabel.textColor = .secondaryLabelColor
        ...
        searchTask = Task { [weak self] in
            ...
            do {
                ...
                await self.diagnosticsLog.record(
                    subsystem: .search,
                    level: .warning,
                    message: Localized.string("Workspace search failed", comment: "Diagnostics log message recorded when a workspace search fails"),
                    context: [
    ...
    private func applyCompletion(_ completion: SearchCompletion) {
        if completion.matchCount == 0 {
            statusLabel.stringValue = Localized.string("No results.", comment: "Status label shown when a workspace search finds no matches")
