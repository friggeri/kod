...
@MainActor
public final class GitDiffViewController: NSViewController {
    enum Mode: Equatable {
        case unified
        case sideBySide
    }

    public struct GitQuickDiffSource {
        public let label: String
        public let diff: GitFileDiff
        public let projection: GitQuickDiffProjection
        public let layer: CodeGutterChange.Layer

        public init(
            label: String,
            diff: GitFileDiff,
            projection: GitQuickDiffProjection,
    ...
    @MainActor
    public final class GitQuickDiffController {
        ...
        public var onOpenFullDiff: ((GitFileDiff) -> Void)?

        public init(documentController: CodeDocumentViewController) {
        ...
        public func update(sources: [GitQuickDiffSource], revealFirstHunk: Bool = false) {
                    ...
                    for deletion in hunk.deletionAnchors {
                        ...
                        changes.append(
                            CodeGutterChange(
                                ...
                                accessibilityLabel: gitUIStrings.string(
        ...
        public func showUnavailable(message: String, diff: GitFileDiff? = nil) {
            guard let documentController else {
                return
            }
            sourcesByProviderID = [:]
        ...
        public func showNextHunk() {
            navigateHunk(by: 1)
        }

        public func showPreviousHunk() {
            navigateHunk(by: -1)
        }

        public func openFullDiff() {
        ...
        private static func accessibilityLabel(
            ...
            switch kind {
            case .added:
                kindLabel = gitUIStrings.string("Added", comment: "Git gutter change kind")
            case .modified:
                kindLabel = gitUIStrings.string("Modified", comment: "Git gutter change kind")
            case .deleted:
                kindLabel = gitUIStrings.string("Deleted", comment: "Git gutter change kind")
            }
    ...
    final class GitQuickDiffPeekViewController: NSViewController {
        ...
        override func loadView() {
            let container = NSView()
            container.wantsLayer = true
            container.layer?.backgroundColor = ThemeColorAppKitBridge
                .nsColor(theme.editor.background).cgColor
            container.layer?.borderColor = ThemeColorAppKitBridge
                .nsColor(theme.git.gutterModified).cgColor
            container.layer?.borderWidth = 1
            container.layer?.masksToBounds = true

            let title = gitUIStrings.string(
                "\(fileName) Git Local Changes (\(providerLabel)) — \(hunkIndex + 1) of \(max(1, hunkCount))",
                comment: "Inline Git diff hunk header"
            )
            titleLabel.stringValue = title
            titleLabel.font = .systemFont(ofSize: 11)
            titleLabel.textColor = ThemeColorAppKitBridge.nsColor(
            ...
            let previousButton = symbolButton(
                "chevron.up",
                label: gitUIStrings.string("Previous Change", comment: "Inline Git diff previous-change button"),
                action: #selector(previousChange(_:))
            )
            let nextButton = symbolButton(
                "chevron.down",
                label: gitUIStrings.string("Next Change", comment: "Inline Git diff next-change button"),
                action: #selector(nextChange(_:))
            )
            let fullDiffButton = symbolButton(
                "rectangle.split.2x1",
                label: gitUIStrings.string(
                    "Open Full Diff",
            ...
            let closeButton = symbolButton(
                "xmark",
                label: gitUIStrings.string("Close Inline Diff", comment: "Inline Git diff close button"),
            ...
            header.layer?.backgroundColor = ThemeColorAppKitBridge
                .nsColor(theme.editor.stickyScopeBackground).cgColor
            ...
            textView.textColor = ThemeColorAppKitBridge.nsColor(
                theme.editor.foreground
            )
        ...
        private func renderHunk() {
            ...
            for (index, line) in hunk.lines.enumerated() {
                ...
                case .added:
                    marker = "+"
                    markerColor = ThemeColorAppKitBridge.nsColor(
                        theme.git.gutterAdded
                    )
                    background = ThemeColorAppKitBridge.nsColor(
                        theme.git.insertedBackground
                    )
                case .removed:
                    marker = "-"
                    markerColor = ThemeColorAppKitBridge.nsColor(
                        theme.git.gutterDeleted
                    )
                    background = ThemeColorAppKitBridge.nsColor(
                        theme.git.removedBackground
                    )
                case .context:
                    marker = " "
                    markerColor = ThemeColorAppKitBridge.nsColor(
                        theme.editor.gutterForeground
                    )
                    background = nil
                ...
                let rowString = NSMutableAttributedString(
                    ...
                    attributes: [
                        ...
                        .foregroundColor: ThemeColorAppKitBridge.nsColor(
                            theme.editor.foreground
                        )
                    ]
                )
                ...
                switch line.kind {
                case .added:
                    intralineBackground = ThemeColorAppKitBridge.nsColor(
                        theme.git.insertedTextBackground
                    )
                case .removed:
                    intralineBackground = ThemeColorAppKitBridge.nsColor(
                        theme.git.removedTextBackground
                    )
    ...
    private let modeControl = NSSegmentedControl(
        labels: [
            gitUIStrings.string("Unified", comment: "Diff view mode segment: unified diff layout"),
            gitUIStrings.string("Side by Side", comment: "Diff view mode segment: side-by-side diff layout")
        ],
        trackingMode: .selectOne,
        target: nil,
        action: nil
    )
    private let statusLabel = NSTextField(labelWithString: gitUIStrings.string("No diff.", comment: "Status text shown when there is no Git diff to display"))
    ...
    public func update(diff: GitFileDiff?) {
        self.diff = diff
        renderCurrentMode()
    }
    ...
    private func renderCurrentMode() {
        guard let diff else {
            textView.string = ""
            statusLabel.stringValue = gitUIStrings.string("No diff.", comment: "Status text shown when there is no Git diff to display")
            return
        }

        if case .binary = diff.content {
            textView.string = gitUIStrings.string(
...
public typealias GitQuickDiffSource = GitDiffViewController.GitQuickDiffSource
public typealias GitQuickDiffController = GitDiffViewController.GitQuickDiffController
