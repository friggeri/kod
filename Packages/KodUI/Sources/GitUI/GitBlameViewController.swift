...
final class GitBlameViewController: NSViewController {
    ...
    override func loadView() {
        ...
        column.title = gitUIStrings.string("Blame", comment: "Column title for the Git blame gutter table")
    ...
    static func gutterText(for line: GitBlameLine) -> String {
        if line.commit.isUncommitted {
            return gitUIStrings.string("Not committed yet", comment: "Git blame gutter text for a line with no committed history yet")
        }
        let formatter = Self.dateFormatter
        let date = formatter.string(from: line.commit.authorTime)
        return "\(line.commit.authorName), \(date) • \(line.commit.summary)"
    }
