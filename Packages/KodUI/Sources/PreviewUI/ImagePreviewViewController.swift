...
final class ImagePreviewViewController: NSViewController {
    ...
    override func loadView() {
        ...
        zoomModeLabel.setAccessibilityLabel(previewUIStrings.string("Zoom level", comment: "Accessibility label for the zoom-level readout in image preview"))
    ...
    private func zoomModeText() -> String {
        switch zoomMode {
        case .fit:
            return previewUIStrings.string("Zoom: Fit to Window", comment: "Zoom-level readout when the image preview is fit to the window")
        case .actualSize:
            return previewUIStrings.string("Zoom: Actual Size", comment: "Zoom-level readout when the image preview is shown at actual size")
        case .custom(let factor):
            return previewUIStrings.string("Zoom: \(Int((factor * 100).rounded()))%", comment: "Zoom-level readout showing the current zoom percentage")
        }
    }
