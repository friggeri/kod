...
final class StructuredTreeRow: NSObject {
    ...
    var displayKey: String {
        key ?? previewUIStrings.string("root", comment: "Display key shown for the root node of a JSON/plist structured-data preview tree")
    }
}
...
final class StructuredDataPreviewViewController: NSViewController {
    ...
    override func loadView() {
        ...
        valueColumn.title = previewUIStrings.string("Value", comment: "Column title for the structured-data preview's value column")
        ...
        let copyValueItem = NSMenuItem(
            title: previewUIStrings.string("Copy Value", comment: "Structured-data preview context menu item that copies the selected node's value"),
            action: #selector(handleCopyValue),
            keyEquivalent: ""
        )
        let copyPathItem = NSMenuItem(
            title: previewUIStrings.string("Copy Key Path", comment: "Structured-data preview context menu item that copies the selected node's key path"),
...
extension StructuredDataPreviewViewController: NSOutlineViewDelegate {
    func outlineView(_ outlineView: NSOutlineView, viewFor tableColumn: NSTableColumn?, item: Any) -> NSView? {
        ...
        if tableColumn.identifier.rawValue == "key" {
            ...
            field.setAccessibilityLabel(previewUIStrings.string("Key: \(row.displayKey)", comment: "Accessibility label for a structured-data preview row's key cell"))
        } else {
            field.stringValue = row.node.isContainer ? row.node.previewText : row.node.previewText
            field.setAccessibilityLabel(previewUIStrings.string("Value: \(row.node.previewText)", comment: "Accessibility label for a structured-data preview row's value cell"))
        }
        return field
    }
}
